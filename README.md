...
| [openbabel](tools/openbabel/SKILL.md) | Use Open Babel via uvx to convert molecular file formats, generate 3D structures from SMILES, render 2D structure images, and prepare Gaussian input files. Use when working across XYZ/MOL/MOL2/PDB/SMILES/Gaussian formats in computational chemistry workflows. | 1.1 | Requires uv and internet access (uses `uvx --from openbabel-wheel obabel ...`). |
<!-- SKILLS_TABLE_END -->
